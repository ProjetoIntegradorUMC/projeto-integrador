...
def get_logs(current_user):
    ...
    if not admin_email or current_user.email != admin_email:
        return (
            jsonify({"error": "Acesso negado. Requer privilégios de administrador."}),
            403,
        )
    ...
    password_reset_requests = PasswordResetLog.query.filter_by(
        event_type="request"
    ).count()
    ...
    recent_security_logs = (
        SecurityLog.query.order_by(SecurityLog.created_at.desc()).limit(20).all()
    )
    recent_password_logs = (
        PasswordResetLog.query.order_by(PasswordResetLog.created_at.desc())
        .limit(20)
        .all()
    )
